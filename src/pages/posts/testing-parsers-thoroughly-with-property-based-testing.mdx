...
Encoding this test in Haskell requires only a simple function.
...
- I wanted QuickCheck to produce _valid_ parse trees, so I had to encode precedence rules for operations and eliminate impossible programs. This meant ensuring that, for example, children of higher precedence operations were always lower precedence operations.
