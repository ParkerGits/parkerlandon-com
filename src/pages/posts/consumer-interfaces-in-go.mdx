...
<YoutubeEmbed embedUrl="https://www.youtube.com/embed/g9x_KMVLPfs?si=OOgAPUU-Qok11JNI" className="my-4" />
...
For example, the `BookController` is handles an HTTP request for all books (e.g., `GET /books`). The signature of `HandleGetAllBooks` reveals that it accepts the HTTP `Request` and `ResponseWriter` types as arguments.
...
The `service` package is responsible for the "business logic." The `controller` package calls into the `service` package with parameters from the HTTP request to construct the appropriate response.

For example, the `BookService` exposes a method `GetAllBooks` that returns a slice of books.
...
Since my simple Book route example doesn't do any data mapping in the `service` layer, the `BookRepository` interface looks the same as the `BookService`.
