...
To summarize, we can parse chained expressions in Haskell by creating a list of functions that map child expressions to newly constructed parent expressions, then thread the original, leftmost expression through those functions to produce the final root expression. This solution exemplifies a more broad pattern of returning and using "setter" functions wherever we might otherwise employ mutability and reassignment in an imperative language.
